...
ROOT_DIR = os.path.realpath(os.path.join(TOOLS_DIR, '..', 'soulou'))
...
def update_(locale=None):
    ...
    cmd = 'xgettext --no-wrap --sort-by-file --default-domain=soulou ' \
          '--copyright-holder="yɛlɛman s.à.r.l" --package-name="soulou" ' \
          '--package-version="%s" ' \
          '--msgid-bugs-address=bugs@yeleman.com ' \
          '--language=Python --keyword=_ --add-comments ' \
          '--output=soulou.pot ' % version + ' '.join(dirs_str)
    return exec_cmd(cmd)


def merge_(locale):
    return exec_cmd('msgmerge --update --no-wrap --sort-by-file ' \
                    '%s/LC_MESSAGES/soulou.po soulou.pot' % locale)


def compile_(locale):
    return exec_cmd('msgfmt --output-file=%s/LC_MESSAGES/soulou.mo ' \
                    '%s/LC_MESSAGES/soulou.po' % (locale, locale))


def new_(locale):
    return exec_cmd('msginit --locale=%s --no-wrap ', \
                    '--output-file=%s/LC_MESSAGES/soulou.po ' \
                    '--input=soulou.pot' % (locale, locale))
